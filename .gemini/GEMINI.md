...
1. Run `python ./scripts/mem.py ctx --agent antigravity --max-tokens 4000`.
...
4. Save checkpoints with `python ./scripts/mem.py cp "what was done. next step" --agent antigravity`.
5. Finish with `python ./scripts/mem.py done "summary"` and `python ./scripts/mem.py log <input> <output> --agent antigravity --model <model>` when token counts are available.
