...
1. Run `python ./scripts/mem.py ctx --agent copilot --max-tokens 4000`.
...
4. Save checkpoints with `python ./scripts/mem.py cp "what was done. next step" --agent copilot`.
5. Finish with `python ./scripts/mem.py done "summary"` and `python ./scripts/mem.py log <input> <output> --agent copilot --model <model>` when token counts are available.
